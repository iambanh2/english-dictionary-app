...
class CategoryManager {
    ...
    private unsubscribeCategories: any = null;
     ...
     * Setup real-time categories listener
     */
    private setupCategoriesListener(): void {
        ...
        try {
            ...
            this.unsubscribeCategories = onSnapshot(q, 
                (querySnapshot) => {
        ...
        categories.forEach(category => {
            ...
            if (card) {
                card.addEventListener('click', (e) => {
                    ...
                    if (!(e.target as Element)?.closest('.category-actions')) {
    ...
    async createCategory(): Promise<void> {
        const nameInput = document.getElementById('category-name-input') as HTMLInputElement;
        const descriptionInput = document.getElementById('category-description-input') as HTMLInputElement;
        const iconSelect = document.getElementById('category-icon-select') as HTMLSelectElement;
        ...
        // Show loading state
        this.showLoading(true);
        const createBtn = document.getElementById('create-category-btn') as HTMLButtonElement;
        if (createBtn) {
     ...
     */
    private validateCategoryData(name: string, description: string): boolean {
        if (!name) {
    ...
    editCategory(categoryId: string): void {
        ...
        const nameInput = document.getElementById(`edit-name-input-${categoryId}`) as HTMLInputElement;
        if (nameInput) {
            nameInput.focus();
            nameInput.select();
        }
    ...
    async saveCategory(categoryId: string): Promise<void> {
        const nameInput = document.getElementById(`edit-name-input-${categoryId}`) as HTMLInputElement;
        const descriptionInput = document.getElementById(`edit-description-input-${categoryId}`) as HTMLInputElement;
        ...
        // Show loading state
        const saveBtn = document.querySelector(`[onclick="categoryManager.saveCategory('${categoryId}')"]`) as HTMLButtonElement;
        if (saveBtn) {
     ...
     */
    private validateCategoryUpdate(categoryId: string, name: string, description: string): boolean {
        if (!name) {
     ...
     */
    private removeEditUI(categoryId: string): void {
     ...
     * Cancel edit
     */
    cancelEdit(categoryId: string, originalName: string, originalDescription: string = ''): void {
        const nameElement = document.getElementById(`name-${categoryId}`);
        const descriptionElement = document.getElementById(`description-${categoryId}`);
    ...
    deleteCategory(categoryId: string, categoryName: string): void {
        ...
        if (modal && message) {
            ...
            (modal as any).categoryId = categoryId;
        }
    }
    ...
    async confirmDelete(): Promise<void> {
        const modal = document.getElementById('delete-modal');
        const categoryId = (modal as any)?.categoryId;
